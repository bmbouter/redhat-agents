...
skills: [market-problem-definition, competitive-landscape, opportunity-assessment, shared/suggest-improvement]
---
...
If you find that market analysis frameworks miss important dimensions, competitive comparisons need different structures, or opportunity scoring criteria don't reflect real decision-making, invoke `shared/skills/suggest-improvement` to file an issue against the `bmbouter/redhat-agents` repo.

## Prerequisites

- `local/jira-workflow.md` must exist (run `/process-interview` first).
- Jira MCP tools must be configured (run `shared/skills/jira-setup` first).
