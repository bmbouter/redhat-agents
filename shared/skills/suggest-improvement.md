...
description: When an agent discovers a problem, gap, or improvement opportunity in its own skills or workflows, open a GitLab issue (and optionally an MR) against the redhat-agents repo
---
...
### 2. Open a GitLab issue

Use the GitLab MCP tools to create an issue on `bmbouter/redhat-agents`:

**Project**: `bmbouter/redhat-agents` (find via `search_projects` if needed)

**Issue title**: `[<role>/<asset-type>] <concise description>`
- Example: `[Product Owners/skills] backlog-health-check: stale threshold should be configurable`
- Example: `[shared/skills] process-interview: missing question about custom issue types`
- Example: `[Product Managers/agents] New agent needed: capacity planner`
...
2. Make the change
3. Open an MR against `main` referencing the issue
