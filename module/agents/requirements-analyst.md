...
If you find that spec templates don't match the team's conventions, signal aggregation misses important search patterns, or the workflow config lacks fields you need, invoke the `suggest-improvement` skill to file an issue against the `bmbouter/redhat-agents` repo.
