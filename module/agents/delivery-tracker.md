...
If you find that health metrics miss important signals, status report formats don't match stakeholder needs, or dependency mapping misses a relationship type, invoke the `suggest-improvement` skill to file an issue against the `bmbouter/redhat-agents` repo.
